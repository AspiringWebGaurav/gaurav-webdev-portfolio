  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        return prev + Math.random() * 15; // Dynamic progress increment
      });
    }, 100);
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadingTimer = setTimeout(() => {
      setIsLoading(false);
    }, 2000 + Math.random() * 1000); // 2-3 seconds dynamic loading

    // Also listen for actual page load events
    const handleLoad = () => {
      setTimeout(() => {
        setIsLoading(false);
      }, 1500); // Minimum 1.5 seconds loading
    };
  ...
  return (
    <>
      {/* Show loader until portfolio is ready */}
      {isLoading && <UniquePortfolioLoader />}
      
      {/* Main portfolio content - always rendered but initially hidden */}
      <main 
        className={`relative bg-black-100 flex justify-center items-center flex-col overflow-hidden mx-auto sm:px-10 px-5 transition-opacity duration-700 ease-in-out ${
