/**
 * Visitor Analytics Aggregates API with 3-LAYER CACHE + SWR
 * Admin-only endpoint for summary statistics and metrics
 * 
 * Cache Strategy:
 * - Memory (60s) → Redis (2min) → Firebase
 * - Stale-while-revalidate for instant response
...
import { cacheGetSWR } from "@/lib/cache";
import { ADMIN_CACHE_KEYS, ADMIN_CACHE_TTL, buildCacheKey } from "@/lib/cache/keys";
import {
  AnalyticsAggregates,
  RegionStat,
  DeviceStat,
...
/**
 * GET - Fetch analytics aggregates with 3-LAYER CACHE + SWR (admin-only)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Build cache key
    const cacheKey = buildCacheKey(ADMIN_CACHE_KEYS.AGGREGATES, timeRangeParam);
    
    // Use 3-layer cache with SWR
    const responseData = await cacheGetSWR(
      cacheKey,
      () => fetchAggregatesFromFirebase(timeRangeParam),
      {
        prefix: '',  // Key already has prefix
        memoryTTL: ADMIN_CACHE_TTL.AGGREGATES_MEMORY,
        redisTTL: ADMIN_CACHE_TTL.AGGREGATES,
        bypass: bypassCache,
      }
    );
...
/**
 * Fetch aggregates from Firebase (called on cache miss)
 */
async function fetchAggregatesFromFirebase(timeRangeParam: string) {
  ...
  // Use deduplication to handle simultaneous requests during cache miss
  ...
  // Return response data (will be cached by cacheGetSWR)
  return {
    success: true,
    data: {
