...
export const SenderIdentitiesView: React.FC = () => {
  const officialSenders = Object.values(ADMIN_MAIL_SENDERS).filter((s) => !s.isLegacy && !s.isNoReply);
    ...
    <div className="space-y-6 font-admin-sans">
      ...
      <div className="p-4 bg-[#FFFFFF] border border-[#E2E8F0] rounded-none sm:rounded-sm shadow-2xs space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-sm text-black">
            Official Sender Registry ({officialSenders.length} Verified Identities)
          </h3>
        </div>
        <p className="font-admin-mono text-xs text-[#64748B] leading-relaxed">
          Primary Domain: <strong className="text-black">gauravpatil.site</strong> (Verified DKIM &bull; DMARC Configured &bull; Merged SPF).
          Outbound delivery is automatically orchestrated across verified sender accounts with cryptographic authentication.
        </p>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-admin-sans font-bold text-xs uppercase tracking-wider text-black">
              Official Identities &mdash; gauravpatil.site
            </h4>
          </div>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="font-admin-mono text-xs text-[#7C3AED] mt-0.5 select-all font-semibold">
                    {sender.email}
                  </p>
                </div>
              ...
              <div className="pt-2 border-t border-[#F1F5F9] font-admin-mono text-[11px] text-[#64748B] flex items-center justify-between">
                <span>Reply-To:</span>
                <span className="text-black font-semibold">{sender.defaultReplyTo}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="p-4 bg-[#F8FAFC] border border-[#E2E8F0] rounded-sm text-xs font-admin-mono text-[#64748B] flex items-start gap-2.5">
        ...
        <p className="leading-relaxed">
          All senders across gauravpatil.site are cryptographically signed with isolated DKIM keys and protected under domain DMARC policy. The delivery pipeline dynamically manages transactional, newsletter, and blog dispatches with automated idempotency locking.
        </p>
      </div>
    </div>
  );
