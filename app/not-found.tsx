        ...
        <div className="relative flex-1 flex flex-col px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12 py-2 sm:py-3 md:py-4">
          ...
          <div className="flex items-center justify-between pb-2 sm:pb-3 border-b border-slate-700/30 flex-shrink-0">
            <div className="flex items-center gap-2 sm:gap-2.5 md:gap-3">
              <div className={`${isMobile ? 'w-7 h-7' : isTablet ? 'w-8 h-8' : 'w-9 h-9'} bg-gradient-to-br from-purple-500 to-blue-600 rounded-lg flex items-center justify-center flex-shrink-0`}>
                <span className={`text-white font-bold ${isMobile ? 'text-xs' : 'text-sm md:text-base'}`}>G</span>
              </div>
              <div className="min-w-0">
                <h2 className={`text-white font-semibold ${isMobile ? 'text-xs' : 'text-sm'} truncate`}>Gaurav Portfolio</h2>
                <p className={`text-slate-400 ${isMobile ? 'text-[9px]' : 'text-[10px] md:text-xs'} truncate`}>Lost in Space</p>
              </div>
            </div>
            ...
            <div className="inline-flex items-center gap-1.5 sm:gap-2 md:gap-3 bg-purple-500/20 px-1.5 sm:px-2 md:px-4 py-1 md:py-2 rounded-lg border border-purple-500/30 flex-shrink-0">
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-purple-500 rounded-full animate-pulse"></div>
              <span className={`${isMobile ? 'text-[9px]' : 'text-[10px] sm:text-xs md:text-sm'} font-medium text-purple-300 whitespace-nowrap`}>404 Not Found</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center py-3 sm:py-4 md:py-6 lg:py-8 overflow-hidden">
            <div className="w-full max-w-7xl mx-auto">
              ...
              <div className="bg-gradient-to-r from-purple-900/40 to-blue-800/40 border border-purple-700/30 rounded-lg md:rounded-xl p-2.5 sm:p-3 md:p-4 mb-3 sm:mb-4 md:mb-6">
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                  <div className={`${isMobile ? 'w-7 h-7' : isTablet ? 'w-9 h-9' : 'w-10 h-10'} bg-purple-500/20 rounded-lg md:rounded-xl flex items-center justify-center border border-purple-500/30 flex-shrink-0`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <h1 className={`${isMobile ? 'text-sm' : isTablet ? 'text-lg' : 'text-xl lg:text-2xl'} font-bold text-white truncate`}>Page Not Found</h1>
                    <p className={`text-purple-300 ${isMobile ? 'text-[10px]' : isTablet ? 'text-xs' : 'text-sm'} truncate`}>The page you're looking for doesn't exist</p>
                  </div>
                </div>
              </div>
                ...
                <div className={`bg-slate-800/50 rounded-lg md:rounded-xl ${isMobile ? 'p-4' : isTablet ? 'p-5' : 'p-6 md:p-7 lg:p-8'} border border-slate-700/50 backdrop-blur-sm flex flex-col justify-between ${isMobile ? 'min-h-[200px]' : isTablet ? 'min-h-[240px]' : 'min-h-[280px] lg:min-h-[320px]'}`}>
                  <div>
                    ...
                    <div className="space-y-2 sm:space-y-2.5 md:space-y-3 mb-4 sm:mb-5 md:mb-6">
                      <div className={`flex items-center gap-2 sm:gap-2.5 md:gap-3 ${isMobile ? 'p-2.5' : 'p-3'} bg-slate-900/40 rounded-lg border border-slate-700/30`}>
                        <div className={`${isMobile ? 'w-1.5 h-1.5' : 'w-2 h-2'} bg-green-400 rounded-full flex-shrink-0`}></div>
                        <div className="flex-1 min-w-0">
                          <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-slate-200 font-semibold truncate`}>Return to Homepage</p>
                          <p className={`${isMobile ? 'text-[10px]' : 'text-xs'} text-slate-400 mt-0.5 truncate`}>Start fresh from the main page</p>
                        </div>
                      </div>
                      <div className={`flex items-center gap-2 sm:gap-2.5 md:gap-3 ${isMobile ? 'p-2.5' : 'p-3'} bg-slate-900/40 rounded-lg border border-slate-700/30`}>
                  ...
                  <div className="space-y-2.5 sm:space-y-3">
                    <button 
                      ...
                      className={`w-full flex items-center justify-center gap-2 ${isMobile ? 'px-4 py-3' : 'px-6 py-3.5'} bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 active:scale-[0.98] text-white font-semibold rounded-lg transition-all shadow-lg shadow-purple-500/50 hover:shadow-purple-600/60 ${isMobile ? 'text-sm' : 'text-base'}`}
                    >
                      <svg className={`${isMobile ? 'w-4 h-4' : 'w-5 h-5'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                      Back to Homepage
                    </button>
