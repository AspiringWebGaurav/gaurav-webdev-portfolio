...
export const AssistantTurnstileGate: React.FC<AssistantTurnstileGateProps> = ({
  ...
  useEffect(() => {
    ...
    // If widget is already initialized and in DOM, only reset if previous attempt ended in ERROR/TIMEOUT
    if (widgetIdRef.current && window.turnstile) {
      if (justOpened && (status === "ERROR" || status === "TIMEOUT")) {
        try {
          window.turnstile.reset(widgetIdRef.current);
          setStatus("READY");
          setErrorMessage(null);
  ...
  }, [isOpen, activeSubView, forcedStatus, status, renderWidget, cleanupWidget]);

  // Unmount-only cleanup (preserves widget across open/close toggles)
  useEffect(() => {
    return () => {
      cleanupWidget();
    };
  ...
  // 6. Dynamic Position Calculation (Anchored above bubble, responsive for all screens)
  const getPopoverStyle = (): React.CSSProperties => {
    if (positionMode === "draggable" && customPosition && typeof window !== "undefined") {
      const isNearTop = customPosition.y < 160;
      const leftClamped = Math.min(Math.max(12, customPosition.x - 140), window.innerWidth - 316);
      return {
        position: "fixed",
        left: `${leftClamped}px`,
    ...
    return {
      ...
      right: "max(12px, calc(1.25rem + env(safe-area-inset-right, 0px)))",
      maxWidth: "calc(100vw - 24px)",
    };
  };
