...
interface AnimatedCharactersLoginLayoutProps {
  children: ReactNode;
...
/**
 * PRESENTATIONAL ONLY - Animated Characters Login Layout
 * This component provides the visual UI/UX wrapper with animated characters.
 * It accepts children (the actual login form) and does NOT contain any auth logic.
 */
export function AnimatedCharactersLoginLayout({
  children,
  passwordLength = 0,
  showPassword = false,
  brandName = "YourBrand",
  ...
  contactLink = "/contact",
  onContactClick,
}: AnimatedCharactersLoginLayoutProps) {
  const [mouseX, setMouseX] = useState<number>(0);
  const [mouseY, setMouseY] = useState<number>(0);
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="relative hidden lg:flex flex-col justify-between bg-gradient-to-br from-primary/90 via-primary to-primary/80 p-12 text-primary-foreground">
        <div className="relative z-20">
          <div className="flex items-center gap-2 text-lg font-semibold">
            <div className="size-8 rounded-lg bg-primary-foreground/10 backdrop-blur-sm flex items-center justify-center">
              {brandIcon || <Sparkles className="size-4" />}
            </div>
            <span>{brandName}</span>
          </div>
        </div>
        ...
        {showFooterLinks && (
          <div className="relative z-20 flex items-center gap-8 text-sm text-primary-foreground/60 hover:text-primary-foreground">
            <a 
      ...
      {/* Right Login Section - Renders children (actual form) */}
      <div className="flex items-center justify-center p-8 bg-white dark:bg-gray-900">
        <div className="w-full max-w-[420px]">
          {/* Mobile Logo */}
          <div className="lg:hidden flex items-center justify-center gap-2 text-lg font-semibold mb-12 text-gray-900 dark:text-white">
            <div className="size-8 rounded-lg bg-primary/10 flex items-center justify-center">
              {brandIcon || <Sparkles className="size-4 text-primary" />}
            </div>
            <span>{brandName}</span>
          </div>

          {/* Render the actual login form as children */}
          {children}
        </div>
      </div>
    </div>
