      ...
      if (result.success) {
        ...
        if (result.orphanedImages.length > 0) {
          ...
          const shouldCleanup = window.confirm(
            `Found ${result.orphanedImages.length} unused image(s) in cloud storage that are not linked to any project.\n\nDo you want to delete them?`
          );
                      ...
                      return (
                        <div key={idx} className="relative group">
                          <div
                            ...
                            onClick={() => {
                              if (!isUsed && (formData.images?.length || 0) < 10) {
                                handleFieldChange("images", [...(formData.images || []), img]);
                                if (!formData.img) handleFieldChange("img", img);
